...
/// duty ([`regime_obc::windows`]), place one window on the longest exterior edge the
...
/// garage. Rules live in `regime_obc::detectors`; placement (room centre) is here.
fn generate_detectors(floors: &[Floor], answers: &Answers) -> Vec<DetectorOut> {
    let s = FEET_TO_MM;
    let co_required = regime_obc::detectors::requires_co_alarm(answers.garage != "none", false);
...
/// live in `regime_obc::electrical`; placement is here.
#[allow(clippy::many_single_char_names)]
fn generate_electrical(floors: &[Floor]) -> Vec<ElectricalOut> {
    use regime_obc::electrical as el;
    ...
    for floor in floors {
        for r in &floor.rooms {
            ...
            if regime_obc::electrical::SWITCH_AT_ROOM_ENTRANCE {
...
fn generate_stairs(
    ...
    for floor in floors {
        ...
        let spec = match mode {
            BuildingMode::Part9 => regime_obc::stairs::solve_stair(ff_mm),
            BuildingMode::Part3 | BuildingMode::Mixed => regime_obc::part3::stairs::solve_public_stair(ff_mm),
        };
...
fn to_json(answers: &Answers, env: Rect, floors: &[Floor]) -> Value {
    ...
    for floor in floors {
        ...
        let mut header = |width_mm: f32, x: f32, y: f32, opening: &str| {
            let (size, review) = regime_obc::headers::header_size_for(width_mm / s, stories_above);
            headers.push(json!({
        ...
        for r in &floor.rooms {
            if regime_obc::windows::requires_egress(&r.room_type)
...
mod tests {
    ...
    fn part3_manifest_emits_public_stair_dimensions() {
        ...
        assert!(
            width_clear >= regime_obc::part3::stairs::PUBLIC_MIN_WIDTH_MM,
            "public stair width_clear {width_clear} mm is below {} mm",
            regime_obc::part3::stairs::PUBLIC_MIN_WIDTH_MM
        );
        assert!(
            tread_run >= regime_obc::part3::stairs::PUBLIC_MIN_TREAD_MM,
            "public stair tread_run {tread_run} mm is below {} mm",
            regime_obc::part3::stairs::PUBLIC_MIN_TREAD_MM
        );
        assert!(
            riser_height <= regime_obc::part3::stairs::PUBLIC_MAX_RISER_MM,
            "public stair riser_height {riser_height} mm exceeds {} mm",
            regime_obc::part3::stairs::PUBLIC_MAX_RISER_MM
        );
    }
    ...
    fn mixed_building_part3_floor_has_public_stairs_and_part9_floor_private() {
        ...
        for s in stairs {
            ...
            assert!(
                tread >= regime_obc::part3::stairs::PUBLIC_MIN_TREAD_MM,
